  ...
  return (
    <section data-p className="cs-next relative overflow-hidden" style={{ "--next": tint } as React.CSSProperties}>
      ...
      <Link
        ...
        className="group relative grid min-h-[100svh] items-center gap-12 px-6 py-[14vh] md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] md:gap-[5vw] md:px-[var(--frame-pad)]"
      >
        <div>
          <p className="font-mono text-[12px] uppercase tracking-[0.3em] text-ink/60">{labels.label}</p>
          <p aria-label={title} className="mt-8 text-[clamp(64px,10vw,200px)] font-bold uppercase leading-[0.84] tracking-[-0.045em] text-ink">
            {lines.map((line) => (
